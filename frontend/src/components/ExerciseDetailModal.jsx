...
function ExerciseDetailModal({ exerciseId, onClose }) {
  ...
  if (error || !exercise) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm" onClick={onClose}>
        <div className="bg-surface rounded-2xl p-8 max-w-md mx-4 text-center" onClick={(e) => e.stopPropagation()}>
          <span className="material-symbols-outlined text-6xl text-red-500 mb-4 block">error</span>
          <p className="text-white text-xl mb-6">{error || 'Exercício não encontrado'}</p>
          <button
            onClick={onClose}
            className="bg-primary hover:bg-primary-hover text-slate-900 font-bold py-3 px-6 rounded-xl transition-all"
          >
            Fechar
          </button>
        </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 overflow-y-auto" onClick={onClose}>
      <div className="bg-background rounded-2xl border border-border-color w-full max-w-3xl my-8 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        {/* Header */}
        <div className="sticky top-0 bg-surface/95 backdrop-blur-sm border-b border-border-color rounded-t-2xl p-4 flex items-center justify-between">
          <h2 className="text-lg font-black text-white">{exercise.nome}</h2>
          <button
            onClick={onClose}
            className="size-10 flex items-center justify-center rounded-full hover:bg-slate-700 text-slate-400 hover:text-white transition-all"
          >
